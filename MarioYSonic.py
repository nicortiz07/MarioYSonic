...
#Canvas
can = Canvas(v,width=1440,height=720)
can.focus_set()
can.config(bg="black")
can.pack()
...
E=[]  #Matriz de enemigos

def load():
    """
    objetivo: Cargar todas las variables almacenadas en el archivo "save.txt" del juego, para cargar la partida
    """
    global v,can,menu1I,menu2I,x1,x2,menu1,menu2,botoJ,botoO,est,est2,mx,my,mar,sx,sy,soc,ya
    global st,ya2,ys2,st2,ys,E,pvm,pvs,ps,pm,time,vida,vidaS,ron,eron,h,go,x1,x2,dif,difr,spow,NMar,NSoc
    arc=open("save.txt","r")
    L=arc.readlines()
    ##
    est = int(L[0])
    ...
    NMar=str(L[27])
    name=""
    for i in range(len(NMar)-1):
        name += NMar[i]
    NMar = name
    NSoc=str(L[28])
    name=""
    for i in range(len(NSoc)-1):
        name += NSoc[i]
    NSoc = name
    E.append([])
    i=0
    for x in range(29,len(L)-1):
        if(L[x]=="!\n"):
            E.append([])
            i+=1
        else:
...
def save(event):
    """
    objetivo: guardar todas las variables en el archivo "save.txt" del juego, para guardar la partida
    """
    global v,can,menu1I,menu2I,x1,x2,menu1,menu2,botoJ,botoO,est,est2,mx,my,mar,sx,sy,soc,ya
    global st,ya2,ys2,st2,ys,E,pvm,pvs,ps,pm,time,vida,vidaS,ron,eron,h,go,x1,x2,dif,difr,spow,NMac,NSoc
    arc=open("save.txt","w")
    arc.write(str(est)+"\n")
    arc.write(str(est2)+"\n")
    arc.write(str(mx)+"\n")
    ...
    arc.write(str(NMar)+"\n")
    arc.write(str(NSoc)+"\n")
    for x in E:   #Codificar la martiz en el texto
        for y in x:
            arc.write(str(y)+"\n")   
        arc.write("!"+"\n")
    arc.close()
...
def reset():
    """
    objetivo: Cambia todas las variables de juego, a su estado defaul, con el fin de reiniciar el juego
    """
...
def Menu():
    """
    objetivo: Crea un menu donde se permite escojer las obciones del juego, como jugar y cargar partida
    """
    global v,can,menu1I,menu2I,x1,x2,menu1,menu2,botoJ
    menu1.destroy()
    menu2.destroy()
    x1 += 10
    x2 -= 10
    ...
    if(x1<0):
        ...
        botol.place(x=570,y=450)

def Level():
    """
    objetivo: muestra en pantalla con una pequeña animacion todos los niveles disponibles en el juego, permitiendo seleccionarlos
    """
    global v,can,f,botoJ,NameM,NameS
    botoJ.destroy()
    NameML=Label(can,text="NOMBRE JUGADOR Mario",bg="black",fg="red",height=3,width=30,font=200)
    NameML.place(x=590,y=150)
    NameM= Entry(can,width=50)
    NameM.place(x=570,y=200)
    NameMS=Label(can,text="NOMBRE JUGADOR Sonic",bg="black",fg="blue",height=3,width=30,font=200)
    NameMS.place(x=590,y=250)
    NameS=Entry(can,width=50)
    NameS.place(x=570,y=300)
    a=[]
    for i in range(0,5):
        a.append([])
        a[i].append(570)
    ...
    a[5][2].destroy()
    a[5][2]=Button(can,command=nivel6,text="Nivel Infinito",bg="black",fg="white",font=100,height=5,width=30)
    a[5][2].place(x=a[5][0],y=a[5][1])
def nivel1():
    """
    objetivo: aplica los valores a las variables para aignar el nivel 1
    """
...
def nivel2():
    """
    objetivo: aplica los valores a las variables para aignar el nivel 2
    """
    global v,can,dif,difr,difc,EN,nv,ron,ronf
    ron=5
    ronf=25
...
def nivel3():
    """
    objetivo: aplica los valores a las variables para aignar el nivel 3
    """
    global v,can,dif,difr,difc,EN,nv,ron,ronf
    ron=11
    ronf=30
...
def nivel4():
    """
    objetivo: aplica los valores a las variables para aignar el nivel 4
    """
    global v,can,dif,difr,difc,EN,nv,ron,ronf
    ron=15
    ronf=40
...
def nivel5():
    """
    objetivo: aplica los valores a las variables para aignar el nivel 5
    """
    global v,can,dif,difr,difc,EN,nv,ron,ronf
    ron=21
    ronf=50
...
def nivel6():
    """
    objetivo: aplica los valores a las variables para aignar el nivel Infinito
    """
    global v,can,dif,difr,difc,EN,nv,ron,ronf
    ron=0
    ronf=-1
...
def Play():
    """
    objetivo: al presionar un nivel esta funcion carga todo las imagenes y sonidos necesarios para iniciar el juego, ademas de llamar a las funciones que ejecutan el mismo
    """
    global v,can,menu1I,menu2I,M,mar,E,soc,puntos,puntom,puntot,po,VM,VS,dif,difr,difc,vida,visaS,go,K,NMar,NSoc
    go=False
    K=[]
    ###
    winsound.PlaySound('Thefatrat.wav',winsound.SND_ASYNC)
    winsound.SND_LOOP
    if(NMar==0 and NSoc==0):
        NMar=NameM.get()
        NSoc=NameS.get()
    ...
    #Nombre
    Label(can,text=NMar,bg="black",fg="red",font=30).place(x=10,y=0)
    Label(can,text=NSoc,bg="black",fg="blue",font=30).place(x=480,y=0)
    #funciones:
    can.focus_set()
    can.bind("<Key>",key3)
    can.bind("<KeyRelease>",key4)
...
def salto():
    """
    objetivo: Permite el salto de mario, hacia la derecha o izquierda en parabola, en el caso que se este moviendo, o vertical, en el caso en el que se encuetre quieto
    """
...
def salto2():
    """
    objetivo: Permite el salto de Sonic, hacia la derecha o izquierda en parabola, en el caso que se este moviendo, o vertical, en el caso en el que se encuetre quieto
    """
...
def parado():
    """
    Objetivo: cada 800 ms retorna a mario a la posicion estandar, cambiando sus estados a estatico
    """
    global v,can,M,mx,my,mar,est,ya2,est2
    if(not("'d'" in K or "'a'" in K)):
...
def parado2():
    """
    Objetivo: cada 800 ms retorna a Sonic a la posicion estandar, cambiando sus estados a estatico
    """
    global v,can,M,sx,sy,soc,ya2,est2
    if(not("'4'" in K or "'6'" in K)):
...
def key():
    """
    objetivo: realizar los movimientos de mario a partir de las teclas que se presionan en el teclado, como lo marca la lista K
    """
...
def key2():
    """
    objetivo: realizar los movimientos de Sonic a partir de las teclas que se presionan en el teclado, como lo marca la lista K
    """
...
def Caida():
    """
    objetivo: genera la caida de mario cuando este se encuentra pisando fuera de una plataforma
    """
...
def Caida2():
    """
    objetivo: genera la caida de Sonic cuando este se encuentra pisando fuera de una plataforma
    """
...
def fin(n=0):
    """
    Objetivo: ocurre cuando el nivel se acaba o pierden, lo que hace es acabar con las funciones del juego y llamar al reinicio del juego, antes cargando un mensaje de muerte en el caso en el que se pierda el juego
    entrada: n: entero que marca si se perdio el juego o se paso el nivel
    """
    global v,can,go
    go=True
    v.after(3000)
...
def muereM():
    """
    objetivo: Se llama en la muerte de mario, este merma una vida y en el caso de no tener llama a fin como la perdida del juego
    """
...
def muereS():
    """
    objetivo: Se llama en la muerte de sonic, este merma una vida y en el caso de no tener llama a fin como la perdida del juego
    """
...
def Enemigos():
    """
    objetivo: manipula toda la matris de enemigos, con el fuin de realizar estas acciones para todos: mover, eliminar, morir, sumar puntos y demas.
    """
    ...
    for x in range(len(E)):
        if(E[x][0] <= 3):
            ...
            for i in range(len(E)):
                if(((max(E[x][1],E[i][1])-min(E[x][1],E[i][1])<=20) and (max(E[x][2],E[i][2])-min(E[x][2],E[i][2]))<=15)and ((E[x][0]+E[i][0])in [2,3,4])):
                    temp=E[i][0]
                    E[i][0]=E[x][0]
                    E[x][0]= temp
    ...
    if(go==False):
        v.after(120,Enemigos)

def creacion(r=-1):
    """
    objetivo: spawnea un enemigo con caracteristicas aleatorias, como el tipo y si es a la izquierda o la derecha
    entrada: r define si la creacion de da por la muerte de un enemigo o por la creacion de uno nuevo, de esta manera se optimiza el espacio de la matriz y por ende lo que tarda en realizar el for de enemigos
    """
    global E,can,EN
...
def sppow():
    """
    objetivo: la funcion maneja todo el sistema del pow, de modo que tenga solo 3 usos, que cambie la imagen y que tenga su efecto en todos los enemigos
    """
...
def ronda():
    """
    objetivo: es de las funciones principales del juego, pues genera la maxima cantidad de enemigos, ademas de organizar el inicio y fin de los niveles, con sus respectivos bonus o power ups.
    """
...
def poderS():
    """
    objetivo: se activa cuando sonic toca la estrella, la funcion genera el poder del rayo de sonic, eliminando a todos los enemigos arriva de el.
    """
    ...
    for i in range(10):
        ...
        v.after(80-i*8)
    for x in range(len(E)):
        if(E[x][1] < sx+50 and E[x][1] + 60 > sx and E[x][2]<= sy + 50): 
            if(E[x][3]==1):
                E[x][4].destroy()
                E[x][4]= Label(can,image=ec4,bg="black")
                E[x][4].place(x=E[x][1],y=E[x][2])
...
def poderM():
    """
    objetivo: se activa cuando mario toca la estrella, la funcion genera el poder del rayo de mario, eliminando a todos los enemigos alfrente de el.
    """
    ...
    if(estT<=2):
        ...
        for i in range(10):
            ...
            v.after(80-i*8)
    for x in range(len(E)):
        if(E[x][2] < my+50 and E[x][2] + 60 > my and ((E[x][1]<= mx and estT<=2)or(E[x][1]>= mx and not(estT<=2)))): 
            if(E[x][3]==1):
                E[x][4].destroy()
                E[x][4]= Label(can,image=ec4,bg="black")
                E[x][4].place(x=E[x][1],y=E[x][2])
...
def estrella(n=4):
    """
    objetivo: generar la estrella cada cierta cantidad de enemigos, y causar qeu al ser tocada por un jugador el optenga un power up, ademas de cambiar su diseño
    """
...
def puntaje():
    """
    objetivo: al llamarla la funcion actualiza la barra de puntaje que se encuetra en la parte superior del juego
    """
